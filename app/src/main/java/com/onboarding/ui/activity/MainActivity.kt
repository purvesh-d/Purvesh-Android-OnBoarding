...
class MainActivity : BaseCompactActivity<ActivityMainBinding , ActivityMainViewModel>(), View.OnClickListener {
    ...
    override fun initializeObserver(viewModel : ActivityMainViewModel) {
        viewModel.apiResponse.observe(this, Observer {
            Alerter.create(this).setText(it).setBackgroundColorRes(R.color.color_error).setIcon(R.drawable.ic_baseline_error_24).show()
        })
    }
        ...
        when(v?.id) {
            R.id.btnLogin -> {
                if (viewModel.isValidDetails()) {
                    Toast.makeText(this,"Validating credential",Toast.LENGTH_LONG).show()
                    viewModel.doLogin()
                }
            }
        }
